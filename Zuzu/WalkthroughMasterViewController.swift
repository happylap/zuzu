...
class WalkthroughMasterViewController: BWWalkthroughViewController {
    ...
    private func displayServiceAgreement() {
        ...
        let agreementStr =
            "1.「豬豬快租」和各物件來源網站並無任何合作關係，所有物件資料並非各租屋網站給予或售予「豬豬快租」。本 App 僅針對網路公開租屋資料做蒐集、索引，以便於房客更快速地一次搜尋租屋物件。\n\n" +
                "2.「豬豬快租」對 App 內展示的資料，並不具任何控制權或所有權，因此也不為租屋物件資料的正確性負任何責任。任何因物件資料錯誤所造成的損害皆與本公司無關。\n\n" +
        "3. 若物件資料內容涉及違法事項，「豬豬快租」收到用戶舉報後會盡力將物件下架，減少對使用者的影響，但是相關的法律責任概由資料原始張貼人負責。"
